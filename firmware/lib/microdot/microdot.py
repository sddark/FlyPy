...
class Microdot:
    ...
    async def start_server(self, host='0.0.0.0', port=5000, debug=False,
        ...
        async def serve(reader, writer):
            ...
            # LOCAL PATCH (not upstream microdot): always close the socket,
            # even when handle_request raises (OSError with an errno outside
            # MUTED_SOCKET_ERRORS, MemoryError mid-write, ...). Without this
            # the connection task dies with the socket open, permanently
            # leaking an lwIP TCP PCB; enough aborted requests (e.g. a fast
            # JSON-polling page) exhaust the pool and the server stops
            # accepting connections while looking perfectly healthy.
            try:
